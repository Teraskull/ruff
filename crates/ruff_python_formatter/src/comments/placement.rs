...
pub(super) fn place_comment<'a>(
    ...
    match comment.enclosing_node() {
        ...
        AnyNodeRef::StmtWhile(ast::StmtWhile {
            test: expr, body, ..
        })
        | AnyNodeRef::StmtIf(ast::StmtIf {
            test: expr, body, ..
        })
        | AnyNodeRef::StmtFor(ast::StmtFor {
            iter: expr, body, ..
        })
        | AnyNodeRef::StmtAsyncFor(ast::StmtAsyncFor {
            iter: expr, body, ..
        }) => handle_terminal_comment(comment, expr, body, locator),
        AnyNodeRef::StmtWith(ast::StmtWith { items, body, .. })
        | AnyNodeRef::StmtAsyncWith(ast::StmtAsyncWith { items, body, .. }) => {
            let item = items.last().unwrap();
            handle_terminal_comment(
                comment,
                item.optional_vars.as_deref().unwrap_or(&item.context_expr),
                body,
                locator,
            )
        }
